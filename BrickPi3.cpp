...
BrickPi3::BrickPi3(uint8_t addr){
  ...
  Address = addr;
  set_motor_limits(PORT_A, 90, 0);
  set_motor_limits(PORT_B, 90, 0);
  set_motor_limits(PORT_C, 90, 0);
  set_motor_limits(PORT_D, 90, 0);
}
...
int BrickPi3::get_sensor(uint8_t port, sensor_val_struct_t &sensor_val_struct){
  ...
  if(SensorType[port_index] == SENSOR_TYPE_TOUCH ||
    ...
    sensor_touch_t &sensvals = (sensor_touch_t&)sensor_val_struct;
    sensvals.pressed = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_ULTRASONIC){
    sensor_ultrasonic_t &sensvals = (sensor_ultrasonic_t&)sensor_val_struct;
    sensvals.cm = raw_value_8;
    sensvals.inch = raw_value_8 / 2.54;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_COLOR_REFLECTED){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.reflected_red = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_COLOR_AMBIENT){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.ambient = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_COLOR_COLOR){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.color = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_ULTRASONIC_LISTEN){
    sensor_ultrasonic_t &sensvals = (sensor_ultrasonic_t&)sensor_val_struct;
    sensvals.presence = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_INFRARED_PROXIMITY){
    sensor_infrared_t &sensvals = (sensor_infrared_t&)sensor_val_struct;
    sensvals.proximity = raw_value_8;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_LIGHT_ON){
    sensor_light_t &sensvals = (sensor_light_t&)sensor_val_struct;
    sensvals.reflected = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_LIGHT_OFF){
    sensor_light_t &sensvals = (sensor_light_t&)sensor_val_struct;
    sensvals.ambient = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_COLOR_RED){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.reflected_red = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_COLOR_GREEN){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.reflected_green = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_COLOR_BLUE){
     sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.reflected_blue = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_NXT_COLOR_OFF){
     sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.ambient = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_GYRO_ABS){
    sensor_gyro_t &sensvals = (sensor_gyro_t&)sensor_val_struct;
    sensvals.abs = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_GYRO_DPS){
    sensor_gyro_t &sensvals = (sensor_gyro_t&)sensor_val_struct;
    sensvals.dps = raw_value_16;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_ULTRASONIC_CM){
    sensor_ultrasonic_t &sensvals = (sensor_ultrasonic_t&)sensor_val_struct;
    sensvals.cm = raw_value_16 / 10.0;
    sensvals.inch = raw_value_16 / 25.4;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_ULTRASONIC_INCHES){
    sensor_ultrasonic_t &sensvals = (sensor_ultrasonic_t&)sensor_val_struct;
    sensvals.cm = raw_value_16 * 0.254;
    sensvals.inch = raw_value_16 / 10.0;
  }else if(SensorType[port_index] == SENSOR_TYPE_CUSTOM){
    sensor_custom_t &sensvals = (sensor_custom_t&)sensor_val_struct;
    sensvals.adc1 = (((spi_array_in[8] & 0x0F) << 8) | spi_array_in[9]);
    sensvals.adc6 = (((spi_array_in[8] >> 4) & 0x0F) | (spi_array_in[7] << 4));
    sensvals.pin5 = (spi_array_in[6] & 0x01);
    sensvals.pin6 = ((spi_array_in[6] >> 1) & 0x01);
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_COLOR_RAW_REFLECTED){
    sensor_color_t &sensvals = (sensor_color_t&)sensor_val_struct;
    sensvals.reflected_red = raw_value_16;
    //sensvals. = raw_value_16_2; not sure what this value is
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_GYRO_ABS_DPS){
    sensor_gyro_t &sensvals = (sensor_gyro_t&)sensor_val_struct;
    sensvals.abs = raw_value_16;
    sensvals.dps = raw_value_16_2;
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_INFRARED_REMOTE){
    sensor_infrared_t &sensvals = (sensor_infrared_t&) sensor_val_struct;
    for(uint8_t v = 0; v < 4; v++){
      switch(spi_array_in[6 + v]){
        case 1:
          sensvals.remote[v] = REMOTE_BIT_RED_UP;
        break;
        case 2:
          sensvals.remote[v] = REMOTE_BIT_RED_DOWN;
        break;
        case 3:
          sensvals.remote[v] = REMOTE_BIT_BLUE_UP;
        break;
        case 4:
          sensvals.remote[v] = REMOTE_BIT_BLUE_DOWN;
        break;
        case 5:
          sensvals.remote[v] = REMOTE_BIT_RED_UP | REMOTE_BIT_BLUE_UP;
        break;
        case 6:
          sensvals.remote[v] = REMOTE_BIT_RED_UP | REMOTE_BIT_BLUE_DOWN;
        break;
        case 7:
          sensvals.remote[v] = REMOTE_BIT_RED_DOWN | REMOTE_BIT_BLUE_UP;
        break;
        case 8:
          sensvals.remote[v] = REMOTE_BIT_RED_DOWN | REMOTE_BIT_BLUE_DOWN;
        break;
        case 9:
          sensvals.remote[v] = REMOTE_BIT_BROADCAST;
        break;
        case 10:
          sensvals.remote[v] = REMOTE_BIT_RED_UP | REMOTE_BIT_RED_DOWN;
        break;
        case 11:
          sensvals.remote[v] = REMOTE_BIT_BLUE_UP | REMOTE_BIT_BLUE_DOWN;
        break;
        default:
          sensvals.remote[v] = 0;
        break;
    ...
    sensor_color_t &sensvals = (sensor_color_t&) sensor_val_struct;
    sensvals.color           = spi_array_in[6];
    sensvals.reflected_red   = ((spi_array_in[ 7] << 2) | ((spi_array_in[11] >> 6) & 0x03));
    sensvals.reflected_green = ((spi_array_in[ 8] << 2) | ((spi_array_in[11] >> 4) & 0x03));
    sensvals.reflected_blue  = ((spi_array_in[ 9] << 2) | ((spi_array_in[11] >> 2) & 0x03));
    sensvals.ambient         = ((spi_array_in[10] << 2) | ( spi_array_in[11]       & 0x03));
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_COLOR_COLOR_COMPONENTS){
    sensor_color_t &sensvals = (sensor_color_t&) sensor_val_struct;
    sensvals.reflected_red   = ((spi_array_in[ 6] << 8) | spi_array_in[ 7]);
    sensvals.reflected_green = ((spi_array_in[ 8] << 8) | spi_array_in[ 9]);
    sensvals.reflected_blue  = ((spi_array_in[10] << 8) | spi_array_in[11]);
    //sensvals. = ((spi_array_in[12] << 8) | spi_array_in[13]); not sure what this value is
  }else if(SensorType[port_index] == SENSOR_TYPE_EV3_INFRARED_SEEK){
    sensor_infrared_t &sensvals = (sensor_infrared_t&)sensor_val_struct;
    for(uint8_t v = 0; v < 4; v++){
      sensvals.heading [v] = spi_array_in[6 + (v * 2)];
      sensvals.distance[v] = spi_array_in[7 + (v * 2)];
    }
  }else if(SensorType[port_index] == SENSOR_TYPE_I2C){
    i2c_struct_t &sensvals = (i2c_struct_t&)sensor_val_struct;
    for(uint8_t b = 0; b < I2CInBytes[port_index]; b++){
      sensvals.buffer_read[b] = spi_array_in[6 + b];
    }
  }
